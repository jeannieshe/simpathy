...
def geminiInteraction(chat, user_input):
    """
    Performs the Gemini API that will produce one AI generated response to an input text.
    Input: input is a text string that is stated by the user.
    Returns: a string that is generated by Gemini.
    """
...
class gemini():
    ...
    @staticmethod
    def chooseScenario():
        gemini.random_scenario = gemini.scenarios[random.randint(0, 9)]
        ...
        gemini.model = genai.GenerativeModel(
            ...
            having a meaningful conversation with the user, or end the conversation after a maximum of 20 interactions, 
            whichever comes first. When concluding the conversation, provide the evaluation. When you output the evaluation, 
            end it with the words 'End of Interview'.""" % gemini.random_scenario
        )

        gemini.chat = gemini.model.start_chat()

        return gemini.random_scenario

    @staticmethod
    def geminiInteraction(user_input):
        """
        Performs the Gemini API that will produce one AI generated response to an input text.
        Input: input is a text string that is stated by the user.
        Returns: a string that is generated by Gemini.
        ...
        # add the model's response to the chat history
        gemini.chat.history.append({"role": "model", "parts": [model_reply]})
